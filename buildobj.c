 ...
 * Porting modifications for 64-bit safety:
...
struct objinfo {
	void	      *	o_prev;		/* previous object info */
	void	      *	o_base;		/* base of object */
	void	      *	o_last;		/* last object that was built */
};
...
#endif
{
	...
	if (heap->b_obj.o_base != NULL) {
		size_t		size = __get_size (heap->b_obj.o_base);
...
#endif
{
	...
	size_t		total;
		...
		if (heap->b_obj.o_base != NULL) {
			...
			total += __get_size (heap->b_obj.o_base);
	...
	gotmem:
		...
		__set_size (heap->b_obj.o_base, __get_size (heap->b_obj.o_base) + size);
...
#if	USE_PROTO
int (FIND_CHUNK) (build_t * heap, VOID * obj)
#else
int
...
#endif
{
	...
	for (scan = heap->b_first ; scan != NULL ; scan = scan->bc_next) {
		void	      *	temp;
		size_t		adjust;
		size_t		obj_size = __get_size (obj);
		...
		adjust = (scan->bc_free + obj_size) & (heap->b_align - 1);
		temp = (void *) (scan->bc_base - obj_size - adjust);

		if (temp == obj && __get_size (temp) == obj_size) {
			heap->b_chunkp = scan;
			scan->bc_free += adjust;
			scan->bc_base -= adjust;
			return 0;
...
#endif
{
	...
	if ((bchunkp = heap->b_chunkp) == NULL || bchunkp->bc_free == 0) {
		bchunkp = BUILD_CHUNK (heap, __get_size (heap->b_obj.o_base) + 1);
		if (bchunkp == NULL)
			return BUILD_NO_MEMORY;
	}
	...
	* bchunkp->bc_base ++ = ch;
	bchunkp->bc_free -= 1;
	__set_size (heap->b_obj.o_base, __get_size (heap->b_obj.o_base) + 1);
...
#endif
{
		...
		if (size + OBJ_OVERHEAD > __get_size (heap->b_obj.o_base))
			return BUILD_SIZE_OVERFLOW;
...
#endif
{
	...
	if (size != NULL)
		* size = __get_size (heap->b_obj.o_base) - OBJ_OVERHEAD;

	heap->b_obj.o_last = heap->b_obj.o_base;
	heap->b_obj.o_base = NULL;

	return (VOID *) ((unsigned char *)heap->b_obj.o_last + OBJ_OVERHEAD);
}
...
#endif
{
	...
	if (heap->b_obj.o_base != NULL)
		return BUILD_OBJECT_BEGUN;
	if ((VOID *) ((unsigned char *)heap->b_obj.o_last + OBJ_OVERHEAD) != base)
		return BUILD_NOT_LAST;
	...
	{
		size_t size = __get_size(heap->b_obj.o_last);
		heap->b_chunkp->bc_base -= size;
		heap->b_chunkp->bc_free += size;
	}
	heap->b_obj.o_last = NULL;
...
#endif
{
	...
	if ((err = BUILD_ALIGN (heap)) != BUILD_OK ||
		...
		* (obj_t *) ((unsigned char *)heap->b_obj.o_base + OBJ_OVERHEAD) = temp;
		if (size > 0 && data != NULL) {
...
#endif
{
	...
	size = __get_size (heap->b_obj.o_prev) - (sizeof (obj_t) + OBJ_OVERHEAD);
	...
	if (size > 0) {
		...
		if (data != NULL) {
			memcpy (data, (unsigned char *) heap->b_obj.o_prev +
				      sizeof (obj_t) + OBJ_OVERHEAD, * sizep);
		}
	}
	...
	size = __get_size (heap->b_obj.o_prev);
	heap->b_obj = * (obj_t *) ((unsigned char *)heap->b_obj.o_prev + OBJ_OVERHEAD);
	heap->b_chunkp->bc_base -= size;
	heap->b_chunkp->bc_free += size;
...
#else
size_t
build_offset ARGS ((heap))
CONST build_t *	heap;
#endif
{
	return (heap == NULL || heap->b_obj.o_base == NULL ?
			0 : __get_size (heap->b_obj.o_base) - OBJ_OVERHEAD);
}
