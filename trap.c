/*
 * Bourne shell signals and traps.
 * Porting modifications (sigset implementation, name2sig):
...
/*
 * Robust sigset implementation using sigaction.
 */
void (*sigset(int sig, void (*disp)(int)))(int)
{
	...
	if (disp == SIG_HOLD) {
		...
		if (sigismember(&omask, sig))
			return SIG_HOLD;
		/* This is a simplification of System V sigset logic */
		return SIG_DFL; 
	}
...
/*
 * Return a signal number for a character-string name.
 */
...
char	      *	name;
{
#define	SIGNAL(val,short,desc)	if (strcmp (name, #short) == 0 || \
				    strcmp (name, #val) == 0) \
					return val;
#include "signal.tab"
#undef	SIGNAL
