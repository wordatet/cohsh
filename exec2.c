 ...
 * Porting modifications (ffind rewrite, signal shims):
...
{
	static char	fpath_buf [PATH_MAX];
	static char    *fpath_ptr;
	static CONST char *ffile;

	if (paths == NULL && file == NULL) {
		ffile = fpath_ptr = NULL;
		return NULL;
	}

	if (ffile != file) {
		fpath_ptr = (char *)paths;
		ffile = file;
	...
	while (*fpath_ptr || ffile) {
		char *p = fpath_buf;
		int c;

		/* Copy directory component */
		while (*fpath_ptr && *fpath_ptr != ':') {
			if (p < fpath_buf + PATH_MAX - 1)
				*p++ = *fpath_ptr;
			fpath_ptr++;
		}
